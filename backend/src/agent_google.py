...
class AceAssistant(Agent):
    def __init__(self) -> None:
        # Use Google TTS instead of OpenAI voice
        super().__init__(
...
Remember: You ARE Ace. You embody Chris's passion, knowledge, and commitment to making IPC the best pickleball community in Indianapolis. Be enthusiastic, helpful, proactive, and always leave them excited to visit!""",
        ...
        logger.info("Ace assistant initialized with Google TTS")
...
async def entrypoint(context: JobContext) -> None:
    """Main entrypoint for the Ace agent with Google TTS"""
    logger.info(f"Starting Ace agent (Google TTS) for room: {context.room.name}")

    # Create the Ace assistant
    ace = AceAssistant()

    # Use Google STT and TTS
    stt = google.STT(
        languages=["en-US"],
        model="chirp",  # Google Chirp model
    )

    tts = google.TTS(
        voice_name="en-US-Neural2-J",  # Natural male voice
        language="en-US",
    )
    ...
    logger.info("Ace agent session started successfully with Google Chirp")
